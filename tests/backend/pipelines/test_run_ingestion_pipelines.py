...
def test_run_all_ingestion_pipelines_runs_each_and_returns_by_name(
    ...
    ingestions = {name: mocker.Mock(return_value=_table(name)) for name in ("a", "b")}
    mocker.patch.dict(runner.INGESTIONS, ingestions, clear=True)

    results = runner.run_all_ingestion_pipelines(CONFIG)

    assert {name: t["dataset"][0].as_py() for name, t in results.items()} == {"a": "a", "b": "b"}
...
def test_run_all_ingestion_pipelines_reraises_worker_errors(mocker: MockerFixture) -> None:
    ...
    mocker.patch.dict(
        runner.INGESTIONS,
        {
            "ok": mocker.Mock(return_value=_table("ok")),
...
def test_main_passes_ingestion_section_of_config(mocker: MockerFixture) -> None:
    """It should load config.yaml and run all ingestions with its `ingestion` section."""
    read_yaml_mock = mocker.patch(
        f"{runner.__name__}.read_yaml", return_value={"ingestion": CONFIG, "other": {}}
    )
