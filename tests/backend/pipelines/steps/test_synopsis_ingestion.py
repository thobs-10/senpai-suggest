...
def test_run_synopsis_ingestion_fetches_with_config_and_validates(
    ...
    table = synopsis_ingestion.run_synopsis_ingestion(ingestion_config)
    ...
    assert table.column_names == ["anime_id", "Name", "Genres", "synopsis"]
    assert table["synopsis"].to_pylist()[0] == "Space bounty hunters."


def test_ingest_synopsis_rejects_missing_name(
    mocker: MockerFixture, raw_synopsis_table: pa.Table
) -> None:
    """It should fail fast when a required Name is null."""
